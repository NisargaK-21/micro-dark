...
Uses database when available, falls back to in-memory data
"""
...
class DataService:
    """Service for retrieving data from database or memory"""
    ...
    def get_overview_stats() -> Dict[str, Any]:
        ...
        try:
            ...
            tactic_count = db.query(Tactic).count()
            db.close()
            
            return {
                "totalCampaigns": campaign_count or 1247,
                "activeVendors": vendor_count or 342,
                "detectedTactics": tactic_count or 89,
                "forumsMonitored": 23,
                "threatLevel": "High",
    ...
    @staticmethod
    def get_vendors(risk_level: Optional[str] = None, limit: int = 50, offset: int = 0) -> List[Dict]:
        """Get vendors list"""
        try:
            db = next(get_session())
        ...
        filtered = default_vendors
        if risk_level:
            filtered = [v for v in default_vendors if v["riskLevel"].lower() == risk_level.lower()]
        
        return filtered[offset:offset + limit]
    
    @staticmethod
    def get_vendor_details(vendor_id: str) -> Optional[Dict]:
        """Get vendor details"""
        try:
            db = next(get_session())
        ...
        # Fallback
        vendors = DataService.get_vendors()
        for v in vendors:
            if v["id"] == vendor_id:
                return v
        return None
    ...
    @staticmethod
    def get_campaign_trends(days: int = 7) -> Dict[str, Any]:
        """Get campaign trends for charts"""
        # Generate trend data based on days
        labels = []
        ...
        return {
            ...
            "datasets": [
                {
                    "label": "New Campaigns",
                    "data": [12, 19, 15, 25, 22, 30, 28, 35][:days],
                ...
                },
                {
                    ...
                    "data": [280, 295, 310, 320, 315, 330, 340, 342][:days],
                ...
                },
                {
                    ...
                    "data": [65, 70, 72, 75, 78, 82, 85, 89][:days],
    ...
    @staticmethod
    def get_tactic_distribution() -> Dict[str, Any]:
    ...
    @staticmethod
    def get_campaign_graph() -> Dict[str, Any]:
